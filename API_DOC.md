...
4. [管理接口（管理员用）](#4-管理接口管理员用)
   ...
   - [4.10 管理员登录](#410-管理员登录)
   - [4.11 管理员登出](#411-管理员登出)
   - [4.12 获取极验配置](#412-获取极验配置)
   - [4.13 获取管理员列表](#413-获取管理员列表)
   - [4.14 创建管理员](#414-创建管理员)
   - [4.15 修改管理员信息](#415-修改管理员信息)
   - [4.16 删除管理员](#416-删除管理员)
   - [4.17 获取 Bot Token 列表](#417-获取-bot-token-列表)
   - [4.18 创建 Bot Token](#418-创建-bot-token)
   - [4.19 修改 Bot Token](#419-修改-bot-token)
   - [4.20 删除 Bot Token](#420-删除-bot-token)
   - [4.21 获取系统配置](#421-获取系统配置)
   - [4.22 更新系统配置](#422-更新系统配置)
   - [4.23 重启服务器](#423-重启服务器)
   - [4.24 获取系统信息](#424-获取系统信息)
   - [4.25 查询审计日志](#425-查询审计日志)
   - [4.26 获取日志统计](#426-获取日志统计)
   - [4.27 获取操作类型](#427-获取操作类型)
5. [审计日志系统](#5-审计日志系统)
...
### 4.17 获取 Bot Token 列表
- **接口地址**: `/api/admin/bots`
- **请求方法**: `GET`
- **鉴权**: **需要（管理员 Token）**
- **需要等级**: 2+
- **说明**: 返回所有 Bot Token 列表（不含实际 token 值）
- **响应示例**:
  ```json
  {
      "success": true,
