...
### 3.4 ~~人机验证接口（极验 GT4）~~ ⚠️ 已废弃
> **废弃说明**: 此接口已不再需要，人机验证已合并到 [3.1 提交申诉](#31-提交申诉) 接口中。请直接使用表单上传方式一次性完成验证和提交。

- **接口地址**: `/api/appeals/verify-captcha`
- **请求方法**: `POST`
- **鉴权**: 不需要
- **状态**: ⚠️ **已废弃** - 保留用于兼容旧版本，新开发请使用 [3.1 提交申诉](#31-提交申诉) 的表单上传方式
- **说明**: 
  - ~~极验 GT4 人机验证服务端验证接口~~
...
### 3.5 ~~上传图片~~ ⚠️ 已废弃
> **废弃说明**: 此接口已不再需要，文件上传已合并到 [3.1 提交申诉](#31-提交申诉) 接口中。请直接使用表单上传方式一次性完成文件上传和申诉提交。
...
| 操作类型 | `details` 字段内容 |
| :--- | :--- |
| `blacklist_add` | `{ "target_id": "用户ID", "user_type": "user/group", "reason": "拉黑原因/备注" }` |
| `blacklist_remove` | `{ "target_id": "用户ID", "user_type": "user/group", "reason": "移除原因" }` |
| `blacklist_update` | `{ "original_user_id": "原ID", "new_user_id": "新ID", "new_reason": "新原因", "updated_fields": ["字段列表"] }` |
