...
4. [管理接口（管理员用）](#4-管理接口管理员用)
   ...
   - [4.6a 获取等级4待确认列表](#46a-获取等级4待确认列表)
   - [4.6b 确认等级4记录](#46b-确认等级4记录)
   - [4.6c 取消等级4待确认](#46c-取消等级4待确认)
   - [4.7 手动移除黑名单](#47-手动移除黑名单管理端)
   - [4.8 修改黑名单条目](#48-修改黑名单条目)
   - [4.9 删除黑名单条目（RESTful风格）](#49-删除黑名单条目restful风格)
   - [4.10 删除申诉](#410-删除申诉管理员用)
   - [4.11 清理已处理申诉](#411-清理已处理申诉)
   - [4.12 管理员登录](#412-管理员登录)
   - [4.13 管理员登出](#413-管理员登出)
   - [4.13a 统一认证登出](#413a-统一认证登出)
   - [4.14 获取极验配置](#414-获取极验配置)
   - [4.15 获取管理员列表](#415-获取管理员列表)
   - [4.16 创建管理员](#416-创建管理员)
   - [4.17 修改管理员信息](#417-修改管理员信息)
   - [4.18 删除管理员](#418-删除管理员)
   - [4.18a 获取个人信息](#418a-获取个人信息)
   - [4.18b 修改个人信息](#418b-修改个人信息)
   - [4.18c 修改密码](#418c-修改密码)
   - [4.19 获取 Bot Token 列表](#419-获取-bot-token-列表)
   - [4.20 创建 Bot Token](#420-创建-bot-token)
   - [4.21 修改 Bot Token](#421-修改-bot-token)
   - [4.22 删除 Bot Token](#422-删除-bot-token)
   - [4.23 获取 Bot Token 原文](#423-获取-bot-token-原文)
   - [4.24 获取系统配置](#424-获取系统配置)
   - [4.25 更新系统配置](#425-更新系统配置)
   - [4.26 重启服务器](#426-重启服务器)
   - [4.27 获取系统信息](#427-获取系统信息)
   - [4.28 查询审计日志](#428-查询审计日志)
   - [4.29 获取日志统计](#429-获取日志统计)
   - [4.30 获取操作类型](#430-获取操作类型)
   - [4.31 获取备份状态](#431-获取备份状态)
   - [4.32 列出所有备份](#432-列出所有备份)
   - [4.33 创建备份](#433-创建备份)
   - [4.34 删除备份](#434-删除备份)
   - [4.35 获取备份配置](#435-获取备份配置)
...
10. [Logto SSO 认证](#10-logto-sso-认证)
    - [10.1 支持的认证方式查询](#101-支持的认证方式查询)
    - [10.2 Logto 登录入口](#102-logto-登录入口)
    - [10.3 Logto 登录回调](#103-logto-登录回调)
    - [10.4 获取 Logto 绑定状态](#104-获取-logto-绑定状态)
    - [10.5 获取 Logto 绑定授权 URL](#105-获取-logto-绑定授权-url)
    - [10.6 Logto 绑定回调](#106-logto-绑定回调)
...
- **参数说明**（表单方式字段名与JSON方式对应）：
  ...
  | **表单特有** ||||
  | `files` | File[] | 否 | 图片文件（可多选），最多3张，最大3MB |
...
| `ai_analysis_delete` | `{ "appeal_id": "申诉ID" }` |
...
| `max_upload_size` | int | 5242880 | 最大上传文件大小（字节，默认3MB） |
...
**配置项说明**:
- `public_url`: 后端 API 地址，用于生成回调地址
- `frontend_url`: 前端地址，用于生成登出后跳转地址
- `secret_key`: Flask Session 密钥
- `logto.endpoint`: Logto 服务地址
- `logto.app_id`: Logto Application ID
- `logto.app_secret`: Logto Application Secret
